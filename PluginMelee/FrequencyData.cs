        ...
        private void ProcessAttackSet(EnumerableRowCollection<AttackGroup> attackSet)
        {
            ...
            foreach (var player in attackSet)
            {
                ...
                if (countAttacks > 0)
                {
                    ...
                    {
                        AppendBoldText("  Spell\n", Color.Blue);
                        if (checkBox2.Checked == true)
                            ShowDetailedDamage(player.Spell.Where(m => (DamageModifier)m.DamageModifier != DamageModifier.MagicBurst));

                        var spellFreq = player.Spell.Where(m => (DamageModifier)m.DamageModifier != DamageModifier.MagicBurst)
                            .GroupBy(m => m.Amount).OrderBy(m => m.Key);
                        ...
                        (player.Spell.Any(n => (DamageModifier)n.DamageModifier == DamageModifier.MagicBurst)))
                    {
                        ...
                        var mbFreq = player.Spell.Where(m => (DamageModifier)m.DamageModifier == DamageModifier.MagicBurst)
                            .GroupBy(m => m.Amount).OrderBy(m => m.Key);

                        ShowFrequency(mbFreq);
                    }
                    ...
                    if (player.Ability.Count() > 0)
                    {
                        ...
                        if (checkBox2.Checked == true)
                            ShowDetailedDamage(player.Ability);

                        var abilFreq = player.Ability
                            .GroupBy(m => m.Amount).OrderBy(m => m.Key);
                    ...
                    if (player.WSkill.Count() > 0)
                    {
                        ...
                        if (checkBox2.Checked == true)
                            ShowDetailedDamage(player.WSkill);

                        var wskillFreq = player.WSkill
                            .GroupBy(m => m.Amount).OrderBy(m => m.Key);

                        ShowFrequency(wskillFreq);
                    }
